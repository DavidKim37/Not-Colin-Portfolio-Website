...
def index():
    return render_template("yourName.html",
                           display="")  # this is the default what is displayed (right now it is nothing)


@app.route("/printName",
           methods=['GET', 'POST'])  # this is is where the website directs to when clicking the submit button
def yourName():
    if request.method == 'POST':  # if the meathod is post
        form = request.form
        returnName = str(form['inputName'])
        # takes the input and converts it to a string, if it was a integer data type be sure to change str to int
...
@app.route("/project/chessv1/")
def string_route():
    return render_template("task.html", data=data.string())


@app.route("/project/tictactoe/")
def billards_route():
